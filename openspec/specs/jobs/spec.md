...
Defines Carbon-ESG's operational maintenance loop: how a user applies to become a worker, how an administrator reviews that application, how `sold` carbon listings flagged with `needs_workers` automatically open a maintenance `WorkerJob`, how an approved worker claims one open job at a time, how the worker submits a single multipart work report (front + back environment photos plus a time-range and description), and how an administrator reviews that report — approving it as the terminal state of the job or rejecting it to bounce the job back to `claimed` for the same worker to resubmit. The capability owns three tables (`worker_applications`, `worker_jobs`, `worker_job_reports`), three state machines, two cross-table side-effect listeners (listing→job on sold, report→job on rejection), a multipart upload surface governed by `FormRequest` validation that runs before any disk write, and four authorization policies. On-chain settlement of report approval is explicitly out of scope; phase-4 grafts the contract call onto the same `transitionTo()` + `DB::transaction()` shape this spec defines.
